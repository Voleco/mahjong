                          ...
                          uint64_t ways) const
{
    ...
    // 2) 判断是否到达叶子：HCost=0 或深度达上限
    if (cur_h == 0 || depth == dep_limit)
    {
        return ways;
    }
...
uint64_t Policy::Get_Score_DFS(const hand_t &start_hand, ResDeck start_deck, const Hand_Evaluator &he, int dep_limit) const
{
    // 我们不想修改用户传进来的hand，所以复制一份
    hand_t temp_hand = start_hand;

    // ways 从1开始
    uint64_t answer = dfs_impl(temp_hand, start_deck, he, 0, dep_limit, 1ULL);
    return answer;
}
